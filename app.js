...
let viewerBodyMat = null; // ref for live color changes on procedural cars
...
function openViewer(car) {
  ...
  updateURL(car.id);
  // Reset color to default for this car
...
function initViewer(car) {
  ...
  if (viewerRenderer) {
    ...
    // Remove only the canvas element, keep the loading div
...
function buildProceduralCar(car) {
  ...
  const isSUV = cat === 'suv';
  const isTruck = cat === 'suv' && car.brand && /Toyota|Jeep|Ford|Land/.test(car.brand);
  const isSports = ['supercar','hypercar','sports','race'].includes(cat);
  const isClassic = cat === 'classic';
  ...
  // Bevel the front edges for a sporty look
  ...
  for (let i = 0; i < lowerPositions.count; i++) {
    ...
    // Taper the front (negative X) slightly
    if (x < -d.bodyLen * 0.4) {
  ...
  // === Mid section (raised hood/cabin area) ===
  ...
  if (isSUV) {
    ...
    cabinShape.lineTo(1.5, ch);
    cabinShape.lineTo(-1.5, ch);
  } else if (isSports) {
    cabinShape.moveTo(-0.8, 0);
    cabinShape.lineTo(1.0, 0);
    cabinShape.lineTo(0.6, ch);
    cabinShape.lineTo(-0.5, ch);
  } else {
    cabinShape.moveTo(-1.0, 0);
    cabinShape.lineTo(1.1, 0);
    cabinShape.lineTo(0.7, ch);
    cabinShape.lineTo(-0.6, ch);
  }
  ...
  // === Roof (for non-glass roof cars) ===
  if (isSUV || cat === 'luxury') {
    const roof = new THREE.Mesh(
      new THREE.BoxGeometry(2.6, 0.08, d.bodyW - 0.2),
      viewerBodyMat
    );
  ...
  wp.forEach(pos => {
    ...
    for (let i = 0; i < spokeCount; i++) {
      ...
      const spoke = new THREE.Mesh(
        new THREE.BoxGeometry(0.04, wheelR * 1.5, 0.06),
        isSports ? chrome : darkPlastic
      );
    ...
    const disc = new THREE.Mesh(
      new THREE.CylinderGeometry(wheelR * 0.55, wheelR * 0.55, 0.03, 16),
      chrome
    );
    ...
    // Wheel arch (dark plastic fender)
  ...
  if (isSports || cat === 'hypercar' || cat === 'race') {
    const splitter = new THREE.Mesh(
      new THREE.BoxGeometry(0.35, 0.05, d.bodyW - 0.1),
      carbonMat
    );
  ...
  const tlBar = new THREE.Mesh(
    new THREE.BoxGeometry(0.04, 0.06, d.bodyW * 0.8),
    taillightMat
  );
  tlBar.position.set(d.bodyLen / 2 + 0.01, d.rideH + d.bodyH * 0.5, 0);
  group.add(tlBar);

  // === Rear wing (for sports/hypercar/race) ===
  if (['supercar','hypercar','sports','race'].includes(cat)) {
    const wing = new THREE.Mesh(
      new THREE.BoxGeometry(0.25, 0.03, d.bodyW * 0.85),
      carbonMat
    );
  ...
  // === Diffuser (for sports cars) ===
  if (isSports) {
    const diff = new THREE.Mesh(
      new THREE.BoxGeometry(0.3, 0.03, d.bodyW - 0.2),
      carbonMat
    );
...
function changeCarColor(hex) {
  ...
  // If showing a procedural car, just change the body material color (no rebuild needed)
  if (viewerBodyMat) {
